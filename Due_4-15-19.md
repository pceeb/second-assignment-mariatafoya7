...
-Open a terminal tab
-input host address "ssh c177-25@hoffman2.idre.ucla.edu"
-input password
2. What is the first thing that you do once you login?  What is the command? 
-input the command "qrsh" to aquire a compute node 
...
-"ls -l" uses long listing format to view date and time 
-"ls -l -h" prints sizes in human readable format, displaying files as 3K instead of 300
...
-Lists things by time change with the most recent first and for each directory lists sub-directories and files
---
...
2. what is ../
-goes up a level from current directory
3. what is ./?
-Couldn't find the answer to this; tried it on the terminal and an error message appeared.
...
$ cd /home/amanda
-The home directory is /Users/amanda
$ cd ../..
-Goes up two levels to /Users
$ cd ~ (Correct)
-Stands for users home directory
$ cd home
-Would go to a diretory called "home" in the current directory
$ cd ~/data/.. (Correct)
$ cd (Correct)
$ cd .. (Correct)
-Goes up one level to /Users/amanda
````
---
### File systems
...
-The touch command generated a new file called "my_file.txt"; the file does show up using GUI file explorer.

 Use 'ls -l' to inspect the files. How large is my_file.txt? 
-0, file is empty

 When might you want to create a file this way?
-To generate an output file that can't otherwise be generated in the system. 
 Can you use touch to edit my_file.txt?
-No, nano has to be used to edit the file.
...
-">" overwrites the file and ">>" appends the string to the existing file.
...
```
  ...
  c. The first three lines and the last two lines of animals.txt (Correct)
...
-The "-i" flag prompts before the removal (Y or N) prior to removing the file. Files are deleted permanently so this flag allows the user to check the file being removed and make sure its the right one. 
5. In the example below, what does cp do when given several filenames and a directory name?
-Copies the files to the indicated directory
```
$ mkdir backup
...
-Gives an error because a directory is not being indicated and the files can't be copied to a directory that doesn't exist. 
```
$ ls -F
```
...
```
a) $ mkdir 2016-05-20  (Correct)
   $ mkdir 2016-05-20/data
   $ mkdir 2016-05-20/data/processed
   $ mkdir 2016-05-20/data/raw 

b) $ mkdir 2016-05-20   (Correct)
   $ cd 2016-05-20
   $ mkdir data
   $ cd data
   $ mkdir raw processed

c) $ mkdir 2016-05-20/data/raw
   $ mkdir 2016-05-20/data/processed
-Gives error because subdirectories aren't created if a directory doesn't exist

d) $ mkdir 2016-05-20
   $ cd 2016-05-20
   $ mkdir data
   $ mkdir raw processed
-Creates "raw" and "processed" at the same level as "data"
```
---
...
- The flag "-n" specifies a numerical order
...
-Uniq only removes adjacent duplicated lines to prevent excess removal in large data sets.
...
-Head outputs the first 5 lines of the animals.txt file. Tail extracts the last 3 lines from those 5 lines. Sort -r sorts the three lines in reverse order and redirects to the final.txt file with the help of ">". File contains: 
2012-11-06, rabbit
2012-11-06. deer
2012-11-05, raccoon
```
...
-Yes, by using ls *A.txt and ls *B.txt
...
-The output from the new commands are separated whereas in the first they weren't.

Under what circumstances would your new expression produce an error message where the original one would not?
-When there are no files ending in A.txt or B.txt
